...
export async function POST(request: NextRequest) {
  ...
  console.log(`📦 Order webhook received: #${order.order_number} (ID: ${order.id})`);

  // ✅ Fire-and-forget to internal processor endpoint
  // Works on ALL environments — Vercel, Node, Docker, VPS
  const baseUrl = process.env.APP_URL || "https://your-domain.com";
  
  fetch(`${baseUrl}/api/process-order-note`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "X-Internal-Secret": process.env.INTERNAL_API_SECRET || "",
  ...
  }).catch((err) => console.error("🔴 Failed to trigger note processor:", err.message));

  // ✅ Respond to Shopify immediately (no waiting)
